...
isom Hs (0 : R) = 0 :=
exists.elim 
    ((isom_is_bijective Hs).right 0) 
    (λ a H, 
    have He : (isom Hs a) * (isom Hs 0) = 0, 
        from (eq_comm.mp H) ▸ (ring.zero_mul (isom Hs 0)),
    (ring.mul_zero a) ▸ ((eq_comm.mp (ring_isom.map_mul Hs a 0)) ▸ He))

lemma isom_zero_iff (Hs : ring_isom R F) {x : R} :
isom Hs x = 0 ↔ x = 0 := 
⟨ λ H, (function.injective.eq_iff (isom_is_bijective Hs).left).mp ((eq_comm.mp (isom_zero Hs)) ▸ H), 
  λ H, (eq_comm.mp H) ▸ (isom_zero Hs) ⟩ 

lemma isom_neg (Hs : ring_isom R F) (x : R) : 
isom Hs (-x) = -isom Hs x :=
have H : isom Hs (-x + x) = 0, 
    from eq_comm.mp (neg_add_self x) ▸ isom_zero Hs,
add_eq_zero_iff_eq_neg.mp ((ring_isom.map_add Hs (-x) x) ▸ H) 

lemma isom_neg_one (Hs : ring_isom R F) : 
isom Hs (-1) = -1 := (ring_isom.map_one Hs) ▸ (isom_neg Hs 1)
...
anti_isom Hs 0 = 0 :=
exists.elim 
    ((anti_isom_is_bijective Hs).right 0) 
    (λ a H, 
...
⟨ λ H, (function.injective.eq_iff (anti_isom_is_bijective Hs).left).mp ((eq_comm.mp (anti_isom_zero Hs)) ▸ H), 
  λ H, (eq_comm.mp H) ▸ (anti_isom_zero Hs) ⟩

lemma anti_isom_neg (Hs : ring_anti_isom R F) (x : R) : 
anti_isom Hs (-x) = -anti_isom Hs x :=
have H : anti_isom Hs (-x + x) = 0, 
    from eq_comm.mp (neg_add_self x) ▸ anti_isom_zero Hs,
add_eq_zero_iff_eq_neg.mp ((ring_anti_isom.map_add Hs (-x) x) ▸ H) 

lemma anti_isom_neg_one (Hs : ring_anti_isom R F) : 
anti_isom Hs (-1) = -1 := (ring_anti_isom.map_one Hs) ▸ (anti_isom_neg Hs 1) 
...
invo Hi x = 0 ↔ x = 0 := anti_isom_zero_iff (Hi.to_ring_anti_isom)
