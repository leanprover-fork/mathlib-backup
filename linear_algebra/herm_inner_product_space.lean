import analysis.normed_space linear_algebra.sesquilinear_form_space data.complex.basic analysis.metric_space
...
noncomputable def norm_inprod {W : Type u} [normed_space ℂ W] : W → W → ℂ := λ (x y : W), ↑(1/4 : ℝ)*(∥x + y∥^2 - ∥x - y∥^2 + I*(∥x + I • y∥^2 - ∥x - I • y∥^2))

--lemma norm_zero {G : Type u} [normed_group G] : ∥(0 : G)∥ = 0 := norm_zero_iff_zero.mpr (refl 0) 

lemma norm_inprod_self {W : Type u} [normed_space ℂ W] (x : W) : 
norm_inprod x x = ∥x∥^2 :=
begin
dunfold norm_inprod,
rw sub_self,
rw norm_zero,
rw of_real_zero,
rw zero_pow 2 (by norm_num),
rw sub_zero,
rw ←norm_smul_I (x - I • x),
rw smul_sub,
rw smul_smul,
rw I_mul_self,
rw neg_one_smul,
...
lemma norm_inprod_smul_left {W : Type u} [normed_space ℂ W] (x y : W) (a : ℂ) (Hp : ∀ (x y : W), ∥x + y∥^2 + ∥x - y∥^2 = 2*∥x∥^2 + 2*∥y∥^2) : 
norm_inprod (a • x) x = ∥x∥^2 :=
begin
dunfold norm_inprod,
repeat {rw ←of_real_pow},
repeat {rw ←of_real_add <|> rw ←of_real_mul <|> rw ←of_real_sub},
have H : ∀ (x y : W), ∥x + y∥^2 = -(∥x - y∥^2) + (2 * ∥x∥ ^ 2 + 2 * ∥y∥^2),
    intros k l,
    rw eq_neg_add_iff_add_eq,
    rw field.add_comm,
    exact Hp k l,
have H1 : ∀ (k l : W), ∥k + l∥^2 - ∥k - l∥^2 = 2*(-(∥k - l∥^2) + ∥k∥ ^ 2 + ∥l∥^2),
    intros,    
    rw field.left_distrib,
    rw field.left_distrib,
    rw field.add_assoc,
...
noncomputable def norm_parallelogram_to_herm {W : Type v} [normed_space ℂ W] (Hp : ∀ (x y : W), ∥x + y∥^2 + ∥x - y∥^2 = 2*∥x∥^2 + 2*∥y∥^2) : 
herm_inner_product_space W := 
{
inprod := norm_inprod,
is_fst_lin := 
begin
...
have Hi : ∀ (a : W), ∥I • a∥ = ∥a∥,
    ...
    unfold norm,
    --dunfold norm,
    --dunfold normed_group.norm,
    --dunfold normed_ring.norm,
    --dunfold normed_field.norm,
    rw abs_I,
    rw field.one_mul,
rw ←Hi (y + I • x),
rw ←Hi (y - I • x),
...
rw I_mul_self,
rw neg_one_smul,
rw ←@norm_neg _ _ (I • y + -x),
rw ←sub_eq_add_neg,
rw neg_sub,
rw sub_eq_add_neg (I • y) (-x),
rw neg_neg,
rw add_comm (I • y) x,
apply complex.ext,
    rw conj_re,
...
split,
    ...
    split,
        intros H,
        rw norm_inprod_self at H,
        rw ←of_real_pow at H,
        rw ←of_real_zero at H,
        rw of_real_inj at H,
        exact (norm_eq_zero x).mp (pow_eq_zero H),
